...
final load which verifies a ticket number. Such a release load access doesn't
exist in the current memory model. Hans Boehm suggests_ using a release
fetch-add of zero, and shows that on x86 the code can be transformed as follows:
...
Similar to the above release fetch-add of zero serving as a release load, one
could also use an acquire exchange when an acquire store is desired.
...
McKenney, and Lawrence Crowl for their review, corrections and ideas.
