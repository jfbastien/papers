...
must be given a specific sub-architecture flag or must defer to feature
detection followed by patching when the program is loaded. This is the case, for
example, for x86's ``LOCK CMPXCHG16B`` instruction which is used to implement
lock-free 16-byte operations.
