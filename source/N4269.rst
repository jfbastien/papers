...
Atomic themselves can be optimized. A non-contentious example is constant
...
Fences are generated as a consequence of std::atomic_thread_fence as well as, on
some architectures, atomic operations. Fences tend to be expensive, and removing
redundant ones as well as positioning them optimally leads to great performance
gains, while keeping the code correct and simple. This is `currently under
review in LLVM`_.
...
accesses. A somewhat surprising example is that the following code can (`and
...
release/acquire memory ordering, can also be transformed as show (but currently
isn’t).
...
teaching the compiler to tread ``std::mutex`` or pthread specially, or by
`making it possible to implement mutexes in pure C++`_. Optimization across
translation unit, such as through link-time optimizations, can also help expose
more opportunities.
...
Spinloop are usually implemented with an acquire load, which are equivalent to a
relaxed load followed by an acquire fence in the loop. On some architecture
implementations it may make sense to sink the fence outside the loop, but how
...
Thanks to Robin Morisset, Dmitry Vyukov, Chandler Carruth and Jeffrey Yasskin
for their review, corrections and ideas.
